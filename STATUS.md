...
### -3. Twitchカスタムエモートがテキスト表示のままになる件の追加調査（デバッグログ確認待ち）

- **経緯:** 2026-07-15、実機検証で「配信者固有のカスタムエモート（例:`sasasfAori`）」が画像化されず
  テキストのまま表示される不具合を報告。調査の結果:
  - `CommentEmote::id`は元々`std::string`で実装済み（`int`ではない）。報告された「IDが数値型で
    パースエラーになる」という仮説は現状のコードには当てはまらなかった
  - **確定した修正**: TwitchスタンプCDNのURLパスが`emotes/v2`という誤ったパスになっていた
    （正しくは`emoticons/v2`）。これは実際のバグとして修正済み（画像が壊れて表示される問題を
    引き起こしうるが、「テキストのまま」という報告症状そのものとは一致しない可能性がある）
  - JS側の境界値ロジック（`start===0`や「コメント全体がエモート」のケース）を再検証したが、
    明確なバグは見つからなかった
  - **有力な仮説（未確定）**: 報告された「sasasfAori」のようなエモートが、Twitch公式の
    サブスクライバー/フォロワーエモートではなく、**BetterTTV (BTTV) や FrankerFaceZ (FFZ)**
    といったサードパーティ（ブラウザ拡張機能ベース）のエモートである可能性がある。これらは
    Twitch IRCサーバーの`emotes`タグには一切含まれないため、本プラグインの実装（IRCタグ解析
    ベース）では原理的に検出できない。バグではなく対応範囲外（スコープ外）の可能性が高い
- **追加した診断ログ**: 上記を確定させるため、C++側（`TwitchPlatform.cpp`、`emotes`タグの生値と
  パース件数をOBSログに出力）とJS側（`overlay.html`、`console.debug`/`console.error`で受信した
  emotesデータと発生エラーを出力）の両方にデバッグログを追加した
- **確認方法:**
  1. 問題のカスタムエモートを含むコメントを再現し、OBSログに
     `[TwitchPlatform] emotes tag='...' -> parsed N entrie(s) for message="..."` が出るか確認
     - **ログが出ない（またはタグ値が空）** → Twitchがそもそもタグを送っていない
       → BTTV/FFZ等のサードパーティエモートである可能性が高い（対応対象外）
     - **ログは出るがparsed数が0** → C++側のパースに問題がある（要修正）
     - **parsed数 > 0だが画像化されない** → ブラウザの開発者ツールのコンソールで
       `console.debug('[emote] twitch emotes received:', ...)` の内容と、
       `console.error('[emote] ...')` が出ていないか確認
  2. 画像URL自体の修正（`emoticons/v2`）により、既存の標準エモート表示が壊れていないことも
     併せて確認する

### -2. YouTubeカスタムエモート辞書機能の実機確認

- **経緯:** 2026-07-15、YouTube Data API v3がカスタムメンバーエモートの画像URLを返さない制約を
  回避するため、配信者が自分でショートコード↔画像をマッピングできる辞書機能を新規実装。
  `WsServer`を拡張し簡易HTTP画像配信（`GET /emotes/<filename>`）にも対応させた。ビルド確認・
  JSトークナイザーの単体テストのみ済み、実機未確認
- **確認方法:**
  1. `ツール → obs-live-hub → オーバーレイ → YouTubeエモート辞書設定` を開き、エクスプローラーから
     画像ファイル（PNG等）をダイアログへドラッグ＆ドロップ→自動的にテーブルへ追加され、
     画像プレビューが表示されることを確認
  2. 追加された行のショートコード欄（初期値はファイル名ベース、例:`kusa.png`→`:kusa:`）を
     実際に使いたい文字列（例:「草」）に編集し、「保存して閉じる」で保存
  3. ブラウザで `http://127.0.0.1:8765/emotes/<CSVに記録されたファイル名>` に直接アクセスし、
     画像が表示される（簡易HTTPサーバーが正しく応答する）ことを確認
  4. YouTubeチャットで登録したショートコードを含むコメントを送信し、`overlay.html`上でテキストの
     代わりに画像が表示されることを確認（Twitch側のコメントでは同じ文字列を送っても画像化
     されない＝プラットフォーム別の適用範囲が正しいことも確認）
  5. 「選択行を削除」で行と対応する画像ファイルの両方が消えることを確認
  6. 存在しないファイル名へのGETリクエスト（例:わざと存在しないURLを叩く）で404が返り
     クラッシュしないことを確認
  7. OBS再起動後も`youtube_emotes.csv`からマッピングが復元され、`youtube_emotes`フォルダの
     画像も保持されていることを確認

### -1. Twitchスタンプ（エモート）画像表示機能の実機確認

- **経緯:** 2026-07-15、コメント本文中のTwitchスタンプ（エモート）を`<img>`画像として`overlay.html`に
  安全に描画する機能を新規実装（`innerHTML`不使用・DOM APIのみ）。ビルド確認・JSトークナイザーの
  ロジック単体テスト（Node.js）のみ済み、実機（実際のTwitchチャットでのスタンプ表示）は未確認
- **確認方法:**
  1. Twitchチャットで実際にスタンプ（例: Kappa, PogChamp、またはチャンネル専用エモート）を含む
     コメントを送信し、`overlay.html`上でテキストではなく画像として表示されることを確認
  2. スタンプが複数個・複数種類混在するコメント、日本語テキストとスタンプが混在するコメント
     （例:「こんにちは Kappa 元気？」）で、画像の位置・前後のテキストが正しく表示されることを確認
     （UTF-16コード単位インデックスをそのまま使う設計のため、日本語混在時のズレがないか特に注視）
  3. スタンプを含まない通常のコメント・YouTubeのコメントが従来通り正しく表示されること（退行がないこと）
  4. ブラウザの開発者ツールでDOM構造を確認し、コメント本文が`innerHTML`ではなく個別の
     テキストノード/`<img>`要素として挿入されていることを確認（XSS対策の実装確認）
  5. 存在しないスタンプID・不正な位置情報が来た場合でもクラッシュ・表示崩れが起きないこと
     （防御的スキップの動作確認。意図的な検証は困難なため通常運用で問題が出ないかを見る）

...
> **⚠️ Google OAuth 同意画面「テスト中」ステータスについて**
>
> Google Cloud Console で OAuth 同意画面を作成した直後は公開ステータスが
> **「テスト中」** になっています。このステータスのままだと、
> **リフレッシュトークンの有効期限が 7 日間** に制限されます。
> 7 日間 OBS を起動しないと再認証が必要になり、YouTube 連携時に
> `HTTP 401` エラー・`invalid_grant` エラーが発生します。
>
> **対応方法（いずれか）:**
>
> - **推奨（個人利用）:** 7 日以内に一度は OBS を起動して YouTube 連携を使う運用にする。
>   再認証は `ツール → obs-live-hub → 接続設定 → YouTube タブ → Googleアカウントと連携`
>   からいつでも可能です。
> - **「アプリを公開」:** Google Cloud Console の OAuth 同意画面で「公開」に変更すると
>   制限が解除されますが、使用スコープによっては Google の審査が必要になる場合があります。
>   個人利用の範囲では上記の運用で十分なため、審査なしで公開する必要がある場合のみ検討してください。
